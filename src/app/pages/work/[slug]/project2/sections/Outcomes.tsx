...
const OUTCOMES = [
  { pct: 30, text: "estimated increase in engagement with pre-booking functionality." },
  { pct: 40, text: "predicted reduction in time spent searching for parking spots." },
  { pct: 50, text: "expected increase in successful first-attempt bookings." },
  { pct: 60, text: "projected decrease in user-reported payment errors." },
];
        ...
        <ul className="flex flex-col gap-[28px] md:gap-[40px] md:w-[740px] min-w-0">
          {OUTCOMES.map((o, i) => (
            <li key={i} className="flex items-center md:items-baseline gap-[16px] md:gap-[15px]">
              {/* Big percentage */}
              <div className="flex-none font-PPMori-SemiBold leading-none tracking-tight text-brand-blue text-[64px] md:text-[96px]">
                {o.pct}%
              </div>
              {/* Description */}
